...
namespace Practical_Assignment
{
    public partial class Registration : System.Web.UI.Page
    {
        ...
        protected int validateInputs(string fName, string lName, string email, string dateOfBirth)
        {
            ...
            var currentAge = currentDate.Year - dob.Year;

            if (dob.Date > currentDate.AddYears(-currentAge)) currentAge--;
            if (currentAge < 16 )
            {
                lbl_errorMsg.ForeColor = Color.Red;
            ...
            if (Regex.IsMatch(FName, @"^([a-zA-Z]+?)([-\s'][a-zA-Z]+)*?$") && Regex.IsMatch(LName, @"^([a-zA-Z]+?)([-\s'][a-zA-Z]+)*?$") && Regex.IsMatch(Email, @"^\w+@[a-zA-Z_]+?\.[a-zA-Z]{2,3}$") && Regex.IsMatch(DateOfBirth, @"^(0[1-9]|[12]\d|3[01])/(0[1-9]|1[0-2])/(\d{2})$") && currentAge >= 16)
            {
                return score;
            }
            return score;
        ...
        protected void btn_register_Click(object sender, EventArgs e)
        {
            ...
            if (tb_dob.Text.ToString().Length != 10)
            {
                lbl_errorMsg.ForeColor = Color.Red;
                lbl_errorMsg.Text = "Date of birth not valid.";
            }
            else
            {
                ...
                bool creditCard = validateCreditCard(tb_creditCardNum.Text.Replace(" ", ""));
        ...
        public void createAccount()
        {
            try
            {
                ...
                {
                    using (SqlCommand cmd = new SqlCommand("INSERT INTO Account VALUES (@Fname,@Lname,@CreditCardNum,@CreditCardSCode,@CardholderName,@ExpiryDate,@PasswordHash,@PasswordSalt,@Email,@DateOfBirth,@AccStatus,@OldPassHash,@OldPassSalt, @PasswordTime, @LockedTime)"))
                    {
                        using (SqlDataAdapter sda = new SqlDataAdapter())
                        {
                            ...
                            DateTime passwordTime = DateTime.ParseExact(time, "dd/MM/yyyy HH:mm", CultureInfo.InvariantCulture);
                            cmd.CommandType = CommandType.Text;
                            cmd.Parameters.AddWithValue("@Fname", tb_fName.Text.Trim());
                            cmd.Parameters.AddWithValue("@Lname", tb_lName.Text.Trim());
                            cmd.Parameters.AddWithValue("@CreditCardNum", Convert.ToBase64String(encryptData(tb_creditCardNum.Text.Replace(" ", "").Trim())));
                            ...
                            cmd.Parameters.AddWithValue("@OldPassHash", ""); 
                            cmd.Parameters.AddWithValue("@OldPassSalt", "");
                            cmd.Parameters.AddWithValue("@PasswordTime", passwordTime);
                            cmd.Parameters.AddWithValue("@LockedTime", SqlDbType.DateTime).Value=DBNull.Value;
                            cmd.Connection = con;
                            con.Open();
                            cmd.ExecuteNonQuery();
                            con.Close();
